...
int32_t aes::init()
{
	...
	{
		status = set_dec_key(c_ctx.user_key, c_ctx.key_length);
	}
	if (status < 0)
	    return status;
	...
	if (c_ctx.mode > 0)
	{
		status = set_initial_vector(c_ctx.iv, c_ctx.iv_length);
		if (status < 0)
			return status;
	}
...
int32_t aes::set_initial_vector(uint8_t *ivec, uint32_t length)
{
...
int32_t aes::set_enc_key(uint8_t *user_key, uint32_t length)
{
	uint32_t *rk;
	uint32_t i = 0;
	uint32_t temp;
	...
	memset(key.rd_key, 0, sizeof(uint32_t) * 56);
	rk = key.rd_key;
...
int32_t aes::set_dec_key(uint8_t *user_key, uint32_t length)
{
	uint32_t *rk;
	uint32_t i, j;
	uint32_t temp;
	...
	status = set_enc_key(user_key, length);
	if (status < 0)
		return status;
...
void aes::encrypt(uint8_t *output, const uint8_t *input)
{
	const uint32_t *rk;
	...
	*/
	s0 = GETU32(input) ^ rk[0];
	s1 = GETU32(input + 4) ^ rk[1];
	s2 = GETU32(input + 8) ^ rk[2];
	s3 = GETU32(input + 12) ^ rk[3];
...
void aes::decrypt(uint8_t *output, const uint8_t *input)
{
	const uint32_t *rk;
	uint32_t s0, s1, s2, s3, t0, t1, t2, t3;
	...
	*output_size = L * block_size;
	...
	if (last)
	{
		...
		for (j = length; j < block_size; j++)
			buffer[j] ^= (uint8_t)(block_size - length);
		encrypt(output, buffer);
		*output_size += block_size;
	}
}
